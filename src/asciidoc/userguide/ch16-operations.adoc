...
values ( [code]+a+ and [code]+b+) are passed in as Tuple fields.
...
information, see the section on <<ch15-advanced.adoc#stream-assertions>>. Below
...
The [classname+FirstNBuffer+ Buffer is provided as an optimized means to
determine the top N elements in a grouping.
