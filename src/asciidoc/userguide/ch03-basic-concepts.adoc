...
<<ch04-tuple-fields.adoc#field-sets, Field Sets>>).
...
Note that with [classname]+Each+, as with other types of pipe, you can specify a
list of fields to output, thereby removing unwanted fields from a stream.
...
existing Tap with new functionality, via a sub-class, and/or adding 'meta-data'
to a Tap instance via the generic [classname]+MetaInfo+ instance field. Further,
