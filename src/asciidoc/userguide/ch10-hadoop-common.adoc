...
use +bin/hadoop+ on Hadoop 1, or +bin/yarn+ on Hadoop 2, to submit the
application to the cluster.

For example, to execute an application stuffed into +your-application.jar+, call
...
$HADOOP_CONF_DIR, only +bin/hadoop+ and +bin/yarn+ do.

It should be noted that even though +your-application.jar+ is passed on the
command line to +bin/hadoop+ or +bin/yarn+, this in no way configures Hadoop to
