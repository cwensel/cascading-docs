...
There are already many SubAssemblies provided by Cascading, see the
<<ch17-subassemblies.adoc#subassemblies,Built-in SubAssemblies>> chapter for
details
...
Typically if an exception is thrown, Cascading will shutdown the complete
executing Flow, and force the [methodname]+Flow.complete()+ method to throw an
exeption client side. Obviously if this exception is not handled, the client
application will exit.

To prevent the shutdown, a trap Tap can be bound to whole branches. And in the
face of an exception, the argument data will be saved to the location specified
by the trap Tap, including any specific diagnostic fields that may aide in
...
<<ch18-best-practices.adoc#handling-bad-data>>.

Optionally diagnostic information, with the given field names, may be captured
along with the argument Tuple.

* [code]+element-trace+ - the file and line number the failed operation was
instantiated

* [code]+throwable-message+ - the [methodname]+Throwable#getMessage()+ value

* [code]+throwable-stacktrace+ - the [methodname]+Throwable#printStackTrace()+
value, cleansed
...
NOTE: The example above is using the Hadoop platform, Cascading local mode
ignores [classname]+Checkpoint+ pipes.
...
consistently and when dealing with complex (non-primitive) data types.
...
see the section on <<ch17-subassemblies.adoc#aggregate-by>>.
