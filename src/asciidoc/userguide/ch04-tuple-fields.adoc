...
TIP: To see _Fields sets_ in action, checkout the section on the
<<ch16-operations.adoc#identity-function,Identity function>>.
...
<<ch05-pipe-assemblies.adoc#each-every>> for details on the different columns
...
* enforcement of canonical representations within the Tuple (prevent a field
  from switching arbitrarily between String and Integer types), and
* to allow for pluggable coercion from one type to another type, even if either
  isn't a Java primitive.
...
Second, the SumBy sub-assembly now has a simpler API and can get the type
...
handed to the planner during runtime.
